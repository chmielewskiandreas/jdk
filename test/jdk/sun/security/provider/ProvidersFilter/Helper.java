        ...
        for (ServiceData svcData : getSvcData(Paths.get(svcDataPath))) {
            ...
            for (String algo : allAlgos) {
                ...
                if (availableInCryptoCheckers &&
                    ...
                    if (svc == null) {
                        // The Security::getProviders API does not support transformations except
                        // when the service is explicitly registered for it.
                        continue;
                    }
                }
                if (filter.indexOf(':') != -1) {
                    // Character not supported for algorithms in Security::getProviders.
                    continue;
                }
